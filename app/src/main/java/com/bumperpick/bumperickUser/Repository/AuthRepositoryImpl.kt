...
class AuthRepositoryImpl(
    ...
    override suspend fun updatefirbase_token(): Result<success_model> {
        return try {
            val token = dataStoreManager.getToken.firstOrNull()
            val vendorId = dataStoreManager.getUserId.firstOrNull()

            // Validate required data
            if (token.isNullOrEmpty() || vendorId.isNullOrEmpty()) {
                return Result.Error("Missing authentication token or vendor ID")
            }

            // Use suspendCoroutine to properly handle Firebase callback in suspend function
            val firebaseToken = suspendCoroutine<String?> { continuation ->
            ...
            if (firebaseToken.isNullOrEmpty()) {
                return Result.Error("Failed to retrieve Firebase token")
            }
            ...
            val apiResult = safeApiCall(
                ...
                errorBodyParser = { errorBody ->
                    try {
                        Gson().fromJson(errorBody, error_model::class.java)
                    } catch (e: Exception) {
                        error_model (message = "Unknown error format: $e")
                    }
                }
            )
            ...
            when (apiResult) {
                is ApiResult.Error -> {
                    Log.e("FirebaseToken", "Failed to send FCM token: ${apiResult.error.message}")
                    Result.Error("Failed to update Firebase token: ${apiResult.error.message}")
                }
                is ApiResult.Success -> {
                    Log.d("FirebaseToken", "FCM token updated successfully: ${apiResult.data.message}")
                    Result.Success(apiResult.data)
                }
            }

        } catch (e: Exception) {
            Log.e("FirebaseToken", "Exception while updating Firebase token", e)
            Result.Error("Failed to update Firebase token: ${e.message}", e)
        }
    }

    override suspend fun sendLocation(lat: Double, long: Double,city: String): Result<success_model> {
        return try {
        ...
        return try {
            val updateResponse = safeApiCall(
                ...
                api = {
                    ...
                    map["city"] = city.toRequestBody("text/plain".toMediaType())
                    ...
                    val dobFormatted = try {
                        ...
                        dob // fallback to original if parsing fails
                    }

                    map["dob"] = dobFormatted.toRequestBody("text/plain".toMediaType())
                    Log.d("DOB",dob)
