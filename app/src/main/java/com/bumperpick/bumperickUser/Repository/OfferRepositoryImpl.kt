...
class OfferRepositoryImpl(
    ...
    override suspend fun notification_destroy(id: String): Result<success_model> {
        val token= dataStoreManager.getToken.firstOrNull()?:return Error("Token not found")

        val response=safeApiCall(
            context = context,
            api = {apiService.notification_destroy(token,id)},
            errorBodyParser = {
                try {
                    Gson().fromJson(it, error_model::class.java)
                } catch (e: Exception) {
    ...
    override suspend fun report(
        ...
        map["reportable_id"]=reportable_id
        map["reportable_type"]=reportable_type
        map["reason"]=reason
        map["details"]=details
        map["token"]=token
        val response=safeApiCall(
            context = context,
            api = { apiService.report_offer(map) },
            errorBodyParser = {
                try {
                    Gson().fromJson(it, error_model::class.java)
                } catch (e: Exception) {
