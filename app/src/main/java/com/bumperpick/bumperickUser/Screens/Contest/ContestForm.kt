...
fun contestForm(
    ...
    var conf_email_error by remember { mutableStateOf<String?>(null) }
    ...
    val registercontest by viewmodel.contestRegistration.collectAsState()

    LaunchedEffect(registercontest) {
        when (registercontest) {
            is UiState.Success -> {
                isSubmitting = false
            ...
            is UiState.Error -> {
                ...
                coroutineScope.launch {
                    snackbarHostState.showSnackbar((registercontest as UiState.Error).message)
                }
            }
            is UiState.Loading -> {
                isSubmitting = true
 ...
 /*   LaunchedEffect(mobile) {
    ...
    fun validateForm(): Boolean {
     ...
     //   mobileError = null
        ...
        when {
            conf_email.isBlank() -> {
                conf_email_error = "Confirm email is required"
                isValid = false
            }
            !Patterns.EMAIL_ADDRESS.matcher(conf_email).matches() -> {
                conf_email_error = "Please enter a valid email address"
                isValid = false
            }
            conf_email.length > 100 -> {
                conf_email_error = "Email must be less than 100 characters"
                isValid = false
            }
            conf_email != email -> {
                conf_email_error = "Email does not match"
                isValid = false
            }
        }
    ...
    fun handleSubmit() {
        if (validateForm() && !isSubmitting) {
            viewmodel.registerForContest(contestId, name, email, "")
        }
    }
                ...
                Column(
                    ...
                    Card(
                        ...
                        Row(
                            ...
                            Text(
                                text = "Please fill in all required details to complete your registration",
                                style = MaterialTheme.typography.bodyMedium,
                                color = blueColor
                            )
                        }
