...
class HomePageViewmodel(val offerRepository: OfferRepository,val eventRepository: Event_campaign_Repository):ViewModel() {
   ...
   private val _history_uiState = MutableStateFlow<UiState<OfferHistoryModel>>(UiState.Empty)
    ...
    private val _event_banner= MutableStateFlow<UiState<List<eventSlider>>>(UiState.Empty)
    val event_banner: StateFlow<UiState<List<eventSlider>>> = _event_banner
    ...
    fun fetchEventBanner() {
        viewModelScope.launch {
            val result = eventRepository.getCityEvent()
    ...
    fun getOffers(
        ...
        viewModelScope.launch {
            ...
            when (result) {
                ...
                is Result.Success -> {
                    Log.d("VIEWMODEL_HOME_DATA", result.data.toString())
                    val filteredData = if (removeads) {
                        result.data.filter { it.is_ads==false }
                    } else {
                        result.data
                    }
                    Log.d("VIEWMODEL_HOME_FILTER_DATA", filteredData.toString())
    ...
    fun free_report_state(){
        viewModelScope.launch {
            _reportOffer_uiState.value=UiState.Empty
        }
    }
    private val _reportOffer_uiState=MutableStateFlow<UiState<success_model>>(UiState.Empty)

    val reportOffer_uiState: StateFlow<UiState<success_model>> =_reportOffer_uiState

    fun reportOffer(reportModel: reportModel,reason:String){
        viewModelScope.launch {
            _reportOffer_uiState.value=UiState.Loading
            val result=offerRepository.report(
                reportable_id = reportModel.id,
                reportable_type = "offer",
                reason = reason,
                details = reason

            )
            _reportOffer_uiState.value=when(result){
                is Result.Error -> UiState.Error(result.message)
                Result.Loading -> UiState.Loading
                is Result.Success -> UiState.Success(result.data)
            }
